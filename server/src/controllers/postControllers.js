...
export const createPost = async (req, res, next) => {
    try {
      ...
      const cleanDescription = purify.sanitize(description, {
        ALLOWED_TAGS: ['p', 'br', 'strong', 'em', 'u', 's', 'blockquote',
                       'ol', 'ul', 'li', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6',
                       'a', 'img', 'pre', 'code'],
        ALLOWED_ATTR: ['href', 'src', 'alt', 'target', 'rel']
      })
      
      if (!cleanTitle || !cleanCategory || !cleanDescription) 
...
export const editPost = async (req, res, next) => {
    try {
        ...
        if (!cleanTitle || !cleanCategory || !cleanDescription || cleanDescription.length < 11)
          return next(new HttpError("Fill in all fields", 422))
